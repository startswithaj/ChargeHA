...
// Plugins that implement multiple roles (Tesla is both vehicle and charger)
// are registered in more than one registry, so the same check arrives twice.
...
// The check's own message wins so a check with several failure modes can say which one it hit; `warningMessage` covers the checks that report no
// detail. A timeout or a rejection is not the check reporting anything — the message there is ours, not the plugin's, so the static text stays.
function messageOf(
  settled: PromiseSettledResult<HealthCheckResult>,
  fallback: string,
): string {
