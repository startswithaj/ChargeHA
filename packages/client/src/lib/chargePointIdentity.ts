...
// `chargerConfig` is user-influenced, so a malformed value must degrade to
// "no id shown" rather than break the page. Empty is absent — never an
...
// Read through the plugin's advertised config key so core code never names
// a plugin's config key itself.
