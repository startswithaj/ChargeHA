...
/**
 * The Tapo charger row this UI acts on.
 *
 * INTERIM. The wizard and settings panel are not yet given a charger id
 * (ChargerEditForm passes only a typeId), so the single Tapo row is used.
 ...
 */
export function useTapoChargerId(): string | undefined {
  ...
  return data?.find((c) => c.chargerAdapterType === "tapo")?.id;
}
