...
describe("OCPP charger plugin", () => {
  ...
  describe("persistTransaction", () => {
    it("writes to the row whose charger_id matches — two OCPP chargers stay separate", async () => {
      const rowA = rowFor("row-a", "Charger A");
      const rowB = rowFor("row-b", "Charger B");
      ...
      const plugin = new OcppChargerPlugin(
        throwingMock<PluginDependencies>("PluginDependencies", {
          ...
          patchChargerConfig: (id, patch) => {
            patchCalls.push({ id, patch });
            return Promise.resolve();
          },
        }),
      );
      const persistTransaction = (plugin as unknown as {
        persistTransaction: (
          chargePointId: string,
          tx: { transactionId: number; meterStartWh: number } | null,
        ) => Promise<void>;
      }).persistTransaction.bind(plugin);

      await persistTransaction("cp-b", { transactionId: 1, meterStartWh: 0 });

      expect(patchCalls.length).toBe(1);
      expect(patchCalls[0].id).toBe("row-b");
    });

    it("logs a warning and does not throw or write for an unknown charge point", async () => {
      let warned = false;
      let patched = false;
      const plugin = new OcppChargerPlugin(
        throwingMock<PluginDependencies>("PluginDependencies", {
          log: throwingMock<Logger>("Logger", {
            info: () => {},
            warn: () => {
              warned = true;
            },
          ...
          dbLog,
          resolveChargerConfigs: () => Promise.resolve([]),
          patchChargerConfig: () => {
            patched = true;
            return Promise.resolve();
          },
      ...
      await persistTransaction("cp-unknown", {
        transactionId: 1,
        meterStartWh: 0,
      });
    ...
    it("patches { active_transaction: null } — the delete, not empty string", async () => {
      const rowA = rowFor("row-a", "Charger A");
      const patchCalls: Array<{ id: string; patch: Record<string, unknown> }> =
        [];
      const plugin = new OcppChargerPlugin(
      ...
      await persistTransaction("cp-a", null);

      expect(patchCalls).toEqual([
        { id: "row-a", patch: { active_transaction: null } },
      ]);
    });
  });
